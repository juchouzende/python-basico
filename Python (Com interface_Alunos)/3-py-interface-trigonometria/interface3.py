...
# Obtém o caminho absoluto para a imagem (seno2.png), funcionando tanto
# rodando o .py normalmente quanto depois de empacotado em PyInstaller.
def resource_path(relative_path):
    # PyInstaller cria um diretório temporário e guarda o caminho em _MEIPASS
    # e usa o caminho absoluto para que a aplicação encontre as imagens
...
class CalculadoraTrigonometrica:
    ...
    def criar_imagem_topo(self):
        try:
            ...
            imagem = imagem.resize((288, 200), Image.Resampling.LANCZOS)
            self.foto_topo = ImageTk.PhotoImage(imagem)
    ...
    def criar_campo_entrada(self):
        ...
        tk.Label(
            frame_entrada, text="Ângulo (0 a 90):", font=("Arial", 14), bg="#f0f0f0"
        ).pack(pady=(0, 5))

        # O register() conecta a função Python validar_entrada ao mecanismo
        # de validação nativo do tkinter.
        validar_comando = self.janela.register(self.validar_entrada)
        ...
        self.entrada_angulo = tk.Entry(
            ...
            fg="red",
            validate="key",
            validatecommand=(validar_comando, "%P")
        )
    ...
    def criar_resultados(self):
        ...
        self.resultado_seno = self.criar_linha_resultado(frame_resultados, 0, "Seno:")
        self.resultado_cosseno = self.criar_linha_resultado(frame_resultados, 1, "Cosseno:")
        self.resultado_tangente = self.criar_linha_resultado(frame_resultados, 2, "Tangente:")

    # Cria um par (label descritivo + label de resultado) numa linha do grid.
    # Retorna o label para podermos alterar o texto depois.
    def criar_linha_resultado(self, frame, linha, texto):
